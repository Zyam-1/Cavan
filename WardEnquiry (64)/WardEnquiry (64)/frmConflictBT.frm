...
Begin VB.Form frmConflictBT 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Patient Details Conflict"
   ClientHeight    =   6960
   ClientLeft      =   1245
   ClientTop       =   840
   ClientWidth     =   8670
   HelpContextID   =   10019
   Icon            =   "frmConflictBT.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6960
   ...
   Begin VB.CommandButton cmdNone 
      ...
      Height          =   525
      Left            =   7020
      Picture         =   "frmConflictBT.frx":0ECA
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   4740
      Width           =   1515
   ...
   Begin VB.CommandButton cmdOK 
      ...
      Height          =   525
      Left            =   5400
      Picture         =   "frmConflictBT.frx":13FC
      Style           =   1  'Graphical
      TabIndex        =   1
      Top             =   4740
      Width           =   1455
   ...
   Begin MSFlexGridLib.MSFlexGrid grd 
      ...
      GridLinesFixed  =   3
      ScrollBars      =   2
      FormatString    =   $"frmConflictBT.frx":192E
   ...
   Begin VB.Image Image3 
      Height          =   720
      Left            =   4860
      Picture         =   "frmConflictBT.frx":19C1
      Top             =   5745
      Visible         =   0   'False
      Width           =   720
   End
   Begin VB.Image Image2 
      Height          =   480
      Left            =   7860
      Picture         =   "frmConflictBT.frx":288B
      Top             =   -60
      Width           =   480
   End
   Begin VB.Image imgRedCross 
      Height          =   225
      Left            =   8040
      Picture         =   "frmConflictBT.frx":3155
      Top             =   6000
      Visible         =   0   'False
      Width           =   210
   End
   Begin VB.Image imgGreenTick 
      Height          =   225
      Left            =   8040
      Picture         =   "frmConflictBT.frx":342B
      Top             =   6210
      Visible         =   0   'False
      Width           =   210
   End
   ...
   Begin VB.Label lblMostRecent 
      BorderStyle     =   1  'Fixed Single
      Caption         =   $"frmConflictBT.frx":3701
      Height          =   855
      Left            =   300
      TabIndex        =   5
      Top             =   5670
   ...
   End
End
Attribute VB_Name = "frmConflictBT"
...
10  If grd.Rows - 1 = 0 Then Exit Sub
20  For i = 1 To grd.Rows - 1
30      grd.Row = i
40      grd.col = 5
50      If grd.CellPicture = imgGreenTick.Picture Then
60          Selected = True
70      End If
80  Next i
90  If Not Selected Then
100     iMsg "Please select at least one demographic entry"
110     Exit Sub
120 End If

130 Me.Hide
140 With frmBTview
        'These are passed to form to display latest patient demographics
150     .RecentChart = pRecentChart
160     .RecentDoB = pRecentDoB
170     .RecentPatName = pRecentPatName

180     .grd.Rows = 2
190     .grd.AddItem ""
200     .grd.RemoveItem 1
210     For i = 1 To grd.Rows - 1
220         grd.Row = i
230         grd.col = 5
240         If grd.CellPicture = imgGreenTick.Picture Then
250             .grd.AddItem grd.TextMatrix(i, 0) & vbTab & _
                             grd.TextMatrix(i, 1) & vbTab & _
                             grd.TextMatrix(i, 2)
260         End If

270     Next i
280     If .grd.Rows > 2 Then
290         .grd.RemoveItem 1
300         .Show 1
310     End If
...
40  For x = 0 To 2
50      grd.col = x
60      grd.CellBackColor = vbYellow
70  Next

80  FillInfo

90  S = "The most recent record for this search is for" & vbCrLf & _
        pRecentPatName & " and is dated " & pRecentDate & "." & vbCrLf
100 If Trim$(pRecentChart) <> "" Then
110     S = S & "Chart Number (MRN) " & pRecentChart & "."
120 Else
130     S = S & "The Chart Number (MRN) was not specified."
140 End If
150 S = S & vbCrLf
160 If IsDate(pRecentDoB) Then
170     S = S & "D.o.B " & pRecentDoB & "."
180 Else
190     S = S & "The Patients D.o.B was not specified."
200 End If
210 lblMostRecent.Caption = S

220 For i = 1 To grd.Rows - 1
230     grd.Row = i
240     grd.col = 5
250     Set grd.CellPicture = imgRedCross.Picture
260 Next i
